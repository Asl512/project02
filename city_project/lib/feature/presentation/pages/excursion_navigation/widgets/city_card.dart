...
class CityCardWidget extends StatelessWidget{
  ...
  const CityCardWidget({Key? key, required this.cityEntiti}) : super(key: key);

  @override
  Widget build(BuildContext context) {
    return Container(
      margin: EdgeInsets.fromLTRB(MediaQuery.of(context).size.width/20-5, 0, MediaQuery.of(context).size.width/20-5, 10),
      child: TextButton(
        onPressed:() async {
          SharedPreferences prefs = await SharedPreferences.getInstance();
          await prefs.setString('city', cityEntiti.id);
          Navigator.pushAndRemoveUntil(context, MaterialPageRoute(builder: (_)=> const Navigation()), (route) => false);
          },
        child: Stack(
            ...
            children: [
              ...
              Stack(
                  ...
                  children: [
                    Container(
                        height: 40,
                        width: MediaQuery.of(context).size.width/3 + cityEntiti.name.length.toDouble()*11,
                        decoration: BoxDecoration(
                            color: Colors.black.withOpacity(0.6),
                            borderRadius: const BorderRadius.only(bottomLeft: Radius.circular(30),topRight: Radius.circular(30))
                        )
                    ),
                    Text(cityEntiti.name.toUpperCase(),style: Montserrat(size: 25,style: Bold,color: White))
                  ]),
            ]
        ),),
    );
